extension JSON {
  ...
  public subscript(index: Index) -> JSON {
    get throws {
      ...
      case (.dictionary(let dictionary), .string(let key)):
        return try value(in: dictionary, for: key)
      case (.dictionary(let dictionary), .integer(let key)):
        return try value(in: dictionary, for: String(key))
      case (.array(let array), .integer(let index)):
  ...
  private func value(in dictionary: [String: JSON], for key: String) throws
    -> JSON
  {
